...
namespace NEsper.Catalyst.Configuration
{
    public class PublisherElement : ConfigurationElement
    {
        ...
        /// </summary>
        /// <returns></returns>
        public IEventPublisherFactory CreatePublisherFactory()
        {
            var factoryType = TypeHelper.ResolveType(Type);
            if (!factoryType.IsImplementsInterface(typeof(IEventPublisherFactory)))
            {
            ...
            var eventPublisherFactory = (IEventPublisherFactory)Activator.CreateInstance(factoryType);
